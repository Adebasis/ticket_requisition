...
class HomeController extends Controller{
	...
	public function profilepage(Request $request){
		...
		$is_maintenance_mode = \DB::table('appsettings')->where('id', 5)->get();
		if($is_maintenance_mode[0]->Value == "1"){
	...
	public function reset_password($id){
		...
		$id = base64_decode($id);
		$id_exist = DB::table('useraccount')->where('id', '=', $id)->count();
		if($id_exist > 0){
			$msg = 'yes';
		}else{
			$msg = 'no';
		}
		return view('reset-password')->with('id_exist', $msg)->with('user_id', $id);
	}
	public function postResetPassword(Request $request){
		...
		if(count($users) > 0){
			...
			return redirect('/reset/password')->with('msg','Please check your mail to reset your password link !!!');
		}else{
			return redirect('/reset/password')->with('error', 'Email address do not match an existing, active account.');
		}
	}
	
	public function postResetPasswordSave(Request $request){
		...
		DB::table('useraccount')->where('id','=', $id)->update(array('password'=>$new_pass));
		
		$res_user = DB::table('useraccount')->select('EmailAddress')->where('id', '=', $id)->get();
		$EmailAddress = $res_user[0]->EmailAddress;
		
		// Mail to user to sent out the password
		$this->sendMail(0, 4, $EmailAddress);
		
		return redirect('reset/password/'.$encryped_id.'/link')->with('msg','');
	...
	public function postNewRequest(Request $request){
		...
		$request_from = $_SERVER['HTTP_HOST'];
		...
		if($front_user_is_admin == "yes"){
			...
			if($approver_level == "0"){
				...
				if($first_approver > 0){
					...
					// Check for second level of approvers in that department or not
					// If available then mail goes to second level of approvers
					// Otherwise mail goes to VP
			...
			// If request made by first approver then mail goes to 2nd approvers
			// If 2nd approvers not found then mail goes to VP
			}else if($approver_level == "first"){
		...
		if($requeststatustype_id == "1"){
			...
			if($no_of_approver_level == ""){
				...
				if($user_approver_level == "first"){
					...
					// if 2nd level of approvers exists
					if(count($res_getmail) > 0){
						foreach($res_getmail as $mail){
							$to_email.= ",".$mail->EmailAddress;
						}
						...
						// if no 2nd approvers found, then sent to all VP
						$res_getmail = DB::select("SELECT EmailAddress FROM useraccount WHERE is_vp=1");
						foreach($res_getmail as $mail){
							$to_email.= ",".$mail->EmailAddress;
						}
	...
	public function sendMail($request_id, $tempate_id, $to_email, $approver_lvl=""){
		...
		//$smpt = DB::table('smtp')->where('server_ip', '=', $server_ip)->get();
		...
		if($tempate_id == "4"){
		...
		}else if($tempate_id == "11"){ // 4 = Reset Password Link Template
			$users = DB::table('useraccount')->select('id','FirstName', 'LastName')->where('EmailAddress', $to_email)->get();
			$user_name = $users[0]->FirstName.' '.$users[0]->LastName;
			$url = url('/').'/reset/password/'.base64_encode($users[0]->id).'/link';
			$email_body = str_replace(array('%USER_NAME%','%URL%'),array($user_name,$url),$input_admin);
		}else if($tempate_id == "12"){ // 4 = Reset Password Link Template
			$users = DB::table('admin')->select('id','user_full_name')->where('email', $to_email)->get();
			$user_name = $users[0]->user_full_name;
			$url = url('/').'/adminpanel/reset/password/'.base64_encode($users[0]->id).'/link';
			$email_body = str_replace(array('%USER_NAME%','%URL%'),array($user_name,$url),$input_admin);
		}else if($tempate_id == "13"){
			$users = DB::table('admin')->select('user_full_name','password')->where('email', $to_email)->get();
			$user_name = $users[0]->user_full_name;
			$password = base64_decode(base64_decode(base64_decode(base64_decode(base64_decode($users[0]->password)))));
			$email_body = str_replace(array('%USER_NAME%','%EMAIL%','%PASSWORD%'),array($user_name,$to_email,$password),$input_admin);			
		}else{
			...
			if($tempate_id == "10" && $reqs[0]->first_approver > 0){
    ...
    public function sendMailold($request_id, $tempate_id, $to_email, $approver_lvl=""){
