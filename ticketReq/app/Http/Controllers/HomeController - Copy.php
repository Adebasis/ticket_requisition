...
class HomeController extends Controller{
	...
	public function postResetPassword(Request $request){
		...
		if(count($users) > 0){
			...
			return redirect('/reset/password')->with('msg','Your password has been reset successfully');
		}else{
	...
	public function postNewRequest(Request $request){
		...
		if($approver_level == "0"){
			...
			$second_approver = Session::get('ecomps_user_second_approver');
		...
		DB::table('logs')->insert(array('user_id'=>$requestor_id,'user_name'=>$requstor_name,'user_type'=>'user','request_id'=>$id,'descr'=>$descr,'created_date'=>$CreateDate));
		// ECOMPS LOGS
		...
		// VC Email
		$vc_mail = DB::table('admin')->select('email')->where('id', 2)->get();
		
		//echo $approver_level;exit;
		
		// If User is Admin, then mail will be sent to fulfiller
		//echo $front_user_is_admin;exit;
		if($front_user_is_admin == "yes"){
			...
			if($approver_level == "0"){
				...
				if($first_approver > 0){
					...
					// Get VC email if First Approver not available
					$to_email = $vc_mail[0]->email;
	...
	public function Post_RequestPageApprover(Request $request){
		...
		if($is_cancel == "yes"){
			...
			DB::update("update request set user_cancel = '1' where id = ?", array($pk_id));
			...
			DB::table('logs')->insert(array('user_id'=>$requestor_id,'user_name'=>$log_approver_name,'user_type'=>'user','request_id'=>$pk_id,'descr'=>$descr,'created_date'=>$Dated));
			// ECOMPS LOGS
		...
		if($requeststatustype_id == "1"){
			...
			if($no_of_approver_level == ""){
				...
				if($user_approver_level == "first"){
					...
					DB::table('logs')->insert(array('user_id'=>$requestor_id,'user_name'=>$log_approver_name,'user_type'=>'first','request_id'=>$pk_id,'descr'=>$descr,'created_date'=>$Dated));
					// ECOMPS LOGS
				...
				if($user_approver_level == "second"){
					...
					$descr = str_replace(array('%APPROVER_NAME%','%REQUESTER_NAME%'),array($log_approver_name,$log_requstor_name),$logs_type[0]->name);
					DB::table('logs')->insert(array('user_id'=>$requestor_id,'user_name'=>$log_approver_name,'user_type'=>'second','request_id'=>$pk_id,'descr'=>$descr,'created_date'=>$Dated));
					// ECOMPS LOGS
		...
		if($requeststatustype_id == "4"){
			...
			// ECOMPS LOGS
			$descr = str_replace(array('%APPROVER_NAME%','%REQUESTER_NAME%'),array($log_approver_name,$log_requstor_name),$logs_type[0]->name);
			DB::table('logs')->insert(array('user_id'=>$requestor_id,'user_name'=>$log_approver_name,'user_type'=>$logs_user_type,'request_id'=>$pk_id,'descr'=>$descr,'created_date'=>$Dated));
			// ECOMPS LOGS
		...
		if($requeststatustype_id == "2"){
			...
			DB::table('logs')->insert(array('user_id'=>$requestor_id,'user_name'=>$log_approver_name,'user_type'=>'fulfiler','request_id'=>$pk_id,'descr'=>$descr,'created_date'=>$Dated));
			// ECOMPS LOGS
